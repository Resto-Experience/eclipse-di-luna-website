...
export async function POST(req: Request) {
  ...
  const errors = collectErrors(
    ...
    validateRequired(body.time, 'time'),
  ...
  const { html, text } = renderSubmission('Private event proposal — Eclipse di Luna', {
    Name: fullName,
    Email: body.email,
    Phone: body.phone,
  ...
  try {
    await sendMail({
      to: destinationFor('proposal'),
      subject: `[Proposal] ${fullName} — ${body.eventType} on ${body.date} @ ${body.location}`,
      replyTo: body.email,
      text,
      html,
    });
  } catch (err) {
    console.error('proposal mail failed', err);
